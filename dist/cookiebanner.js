(function(context) {
    ...
    var Utils = {
        merge: function() {
            var obj = {}, i = 0, al = arguments.length, key;
            if (0 === al) {
                return obj;
            }
            for (;i < al; i++) {
        ...
        fade_in: function(el) {
            if (el.style.opacity < 1) {
                el.style.opacity = (parseFloat(el.style.opacity) + .05).toFixed(2);
        ...
        camelize: function(str) {
            var separator = "-", match = str.indexOf(separator);
            while (match != -1) {
                var last = match === str.length - 1, next = last ? "" : str[match + 1], upnext = next.toUpperCase(), sep_substr = last ? separator : separator + next;
    ...
    Cookiebanner.prototype = {
        ...
        build_viewport_mask: function() {
            ...
            if (true === this.options.mask) {
                ...
                var mask_markup = '<div id="cookiebanner-mask" style="' + "position:fixed;top:0;left:0;width:100%;height:100%;" + "background:" + bg + ";zoom:1;filter:alpha(opacity=" + mask_opacity * 100 + ");opacity:" + mask_opacity + ";" + "z-index:" + this.options.zindex + ';"></div>';
        ...
        insert: function() {
            ...
            function on(el, ev, fn) {
                var add = el.addEventListener ? "addEventListener" : "attachEvent", pre = el.addEventListener ? "" : "on";
